...
twitter.tweet = function(msg) {
	...
	if (!twitter.isAuthenticated()) {
		twitter.authenticate(function(error, results){
			if ( !error ) {
				twitter.tweet(msg);				
			} else {
				return null;
	...
	var authorizationHeader = buildAuthorizationHeader([], httpMethod, url);
 	...
 	client.send({status: msg.status, "media[]": msg.image});
};
...
function buildAuthorizationHeader(parameters, method, url) {
	parameters.push({"name": "oauth_consumer_key", "value": twitter.consumerKey});
	
	if (twitter.accessToken) {
		parameters.push({"name": "oauth_token", "value": twitter.accessToken});
	} else if (twitter.requestToken) {
		parameters.push({"name": "oauth_token", "value": twitter.requestToken});
	}
	
	parameters.push({"name": "oauth_nonce", "value": CryptoJS.lib.WordArray.random(128/8).toString()});
	parameters.push({"name": "oauth_timestamp", "value": Math.floor(new Date().getTime() / 1000)});
	parameters.push({"name": "oauth_signature_method", "value": "HMAC-SHA1"});
	parameters.push({"name": "oauth_version", "value": "1.0"});
...
function buildSignature(parameters, method, url) {
	...
	if (twitter.accessTokenSecret) {
		signingKey += encodeURIComponent(twitter.accessTokenSecret);
	} else if (twitter.requestTokenSecret) {
		signingKey += encodeURIComponent(twitter.requestTokenSecret);
	} 
 	...
 	var client = Ti.Network.createHTTPClient({
    	onload : function(e) {
    		...
    		this.responseText.split("&").forEach(function(kvp){
    			...
    			if (splitKvp[0] === "oauth_token") {
					twitter.requestToken = splitKvp[1];	
				} else if (splitKvp[0] === "oauth_token_secret") {
					twitter.requestTokenSecret = splitKvp[1];
				}		
    		});
    		callback(null, null);
     	},
...
function authorize(callback) {
	var webview = Titanium.UI.createWebView({url:'https://api.twitter.com/oauth/authenticate?oauth_token=' + twitter.requestToken});
	...
	webview.addEventListener("beforeload", function(e){
		...
		if ( /^https:\/\/instafortune\.com\/oauth\/callback\?oauth_token=.*/.test(e.url)) {
			e.url.replace("https://instafortune.com/oauth/callback?", "").split("&").forEach(function(kvp){
				var splitKvp = kvp.split("=");
    			 if (splitKvp[0] === "oauth_verifier") {
					twitter.oauthVerifier = splitKvp[1];
				}			
			});
			window.close();
			callback(null, null);	
...
function access_token(callback) {
	...
	var authorizationHeader = buildAuthorizationHeader([], httpMethod, url);
 	...
 	var client = Ti.Network.createHTTPClient({
    	onload : function(e) {
    		this.responseText.split("&").forEach(function(kvp){
    			var splitKvp = kvp.split("=");
    			if (splitKvp[0] === "oauth_token") {
					twitter.accessToken = splitKvp[1];	
				} else if (splitKvp[0] === "oauth_token_secret") {
					twitter.accessTokenSecret = splitKvp[1];
				}		
    		});
    		console.log("derp" + e);
    		callback(null, null);
 	...
 	client.send({"oauth_verifier": twitter.oauthVerifier});					
